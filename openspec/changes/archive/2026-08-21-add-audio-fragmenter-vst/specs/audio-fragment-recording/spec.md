...
- **Given** ein Host mit beliebigen Blockgrößen, **when** identisches Audio zweimal verarbeitet wird, **then** sind Grenzen und Ausgaben deterministisch.
- **Given** Wet=100 % und noch kein vollständiges Fragment, **when** Audio verarbeitet wird, **then** ist die Ausgabe stumm.
- **Given** eine Längenänderung mitten im Fragment, **when** die nächste Grenze erreicht wird, **then** wird die neue Länge erst dort verwendet.
- **Given** ein State-Roundtrip, **when** der gespeicherte State restauriert wird, **then** entsprechen beide Parameter den gespeicherten Werten.
